...
class FeatureCombination:
    ...
    def loadjson_a(self, project):
        ...
        save_path = os.path.join(conf.local_path + 'defect_features/utils/data_tmp', project + '_a.csv')
    ...
    def loadjson_v(self, project):
        ...
        df = pd.DataFrame(commits_dict).T
        save_path = os.path.join(conf.local_path+'defect_features/utils/data_tmp',project + '_v.csv')
    ...
    def combination(self, project):
        read_path_v = os.path.join(conf.local_path+'defect_features/utils/data_tmp',project + '_v.csv')
        df_v = pd.read_csv(read_path_v).set_index('commit_id').T
        dict_v = df_v.to_dict()
        read_path_a = os.path.join(conf.local_path+'defect_features/utils/data_tmp', project + '_a.csv')
        ...
        save_path = os.path.join(conf.local_path+'defect_features/report',project+'.csv')
    ...
    def one_featureCombination(self, commithash):
        for p in conf.projects:
