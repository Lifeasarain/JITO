...
class Changelines:
    ...
    def getChangelines(self, project):
        ...
        lines_path = conf.local_path+"train/diff.txt"
        out = subprocess.check_output(cmd, shell=True).decode('utf-8', errors='ignore')
        # print(out)
        with open(lines_path, 'w', ) as f_obj2:
            f_obj2.write(out)
        fopen = open(conf.local_path+"train/diff.txt", 'r')
        ...
        for index in range(len(lines)):
            ...
            if ('diff' == lines[index][0:4]):
                ...
                line_numbers = []
            if('@@' == lines[index][0:2]):
                projectfile = open(target_path+related_path, 'r')
        ...
        with open(conf.local_path+"train/test.txt", 'w', ) as fl:
            fl.writelines(json_str)

        with open(conf.local_path+"train/javafile.java", 'w', ) as jf:
