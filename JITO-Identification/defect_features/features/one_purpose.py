...
class PurposeFeatures(GitOneCommitFeatures):
...
def extract_one_to_db_obj(project):
    ...
    for pf_obj in pf_objs:
        ...
        if pf_obj.bug_fix_files != None:
            pf_obj.bug_fix_files = json.dumps(pf_obj.bug_fix_files)
        #change object/features.py PurposeFeatures object to db object
        pf_dict = {'project': getattr(pf_obj, 'project'), 'commit_id': getattr(pf_obj, 'commit_id'),
                   'time_stamp': getattr(pf_obj, 'time_stamp'),
                   'is_fix': getattr(pf_obj, 'is_fix'), 'classification': getattr(pf_obj, 'classification'),
                   'linked': getattr(pf_obj, 'linked'),
                   'contains_bug': getattr(pf_obj, 'contains_bug'), 'fix_by': getattr(pf_obj, 'fix_by'),
                   'fixes': getattr(pf_obj, 'fixes'),
                   'buggy_lines': getattr(pf_obj, 'buggy_lines'), 'block': getattr(pf_obj, 'block'),
                   'critical': getattr(pf_obj, 'critical'),
                   'major': getattr(pf_obj, 'major'), 'block_total': getattr(pf_obj, 'block_total'),
                   'critical_total': getattr(pf_obj, 'critical_total'),
                   'major_total': getattr(pf_obj, 'major_total'), 'file_name_stat': getattr(pf_obj, 'file_name_stat'),
                   'find_interval': getattr(pf_obj, 'find_interval'),
                   'la': getattr(pf_obj, 'la'), 'ld': getattr(pf_obj, 'ld'),
                   'fix_file_num': getattr(pf_obj, 'fix_file_num'),
                   'bug_fix_files': getattr(pf_obj, 'bug_fix_files'), 'rules': getattr(pf_obj, 'rules'),
