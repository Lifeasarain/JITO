rm(list=ls())
require(effsize) 
#root <- "/Users/lifeasarain/Desktop/JITO/JIT-Identification/defect_features/report/"
#root <- "/home/qiufc/Python/JIT_defect_prediction_code/"
#remain_features <- c("commit_id", "time_stamp", "creation_time", "is_fix", "ns", "nd", "nf", "entropy", "la", "ld", "lt", "ndev", "age", "nuc", "exp", "rexp", "sexp", "contains_bug")
...
build_model <- function(projectname, root){
  #root <- "/Users/lifeasarain/Desktop/JITO/JIT-Identification/"
  #projectname <- "druid"
  # result preparation
  result_frame <- NULL
  fn <- paste(c(root,"defect_features/report/",projectname,".csv"), collapse="")
  ...
  if (length(which(train_data$is_fix == '1'))==0){
    ...
    model_features2 <- c("ns", "nd", "nf", "entropy", "la", "ld", "lt", "ndev", "age", "nuc", "exp", "rexp", "sexp")
  ...
  save(fit, file = paste(root,"R/learnedModel.RData",sep = "", collapse=""))
}
